...
```{r, eval = FALSE}
.tmp <- foreach(wt = c("jack", "jack_wt"), .combine = bind_rows) %:%
  foreach(i = 1:100, .combine = bind_rows, .errorhandling = "remove") %do% {
    jack <- readRDS(sprintf("Output/test_non_null/2016-11-30/out/%s.%s.rds", wt, i))
    jack %>% 
      as.data.frame() %>% 
      tibble::rownames_to_column(var = "group") %>% 
      tidyr::gather("var", "value", -group) %>% 
...
.tmp <- foreach(r = c("emp", "true"), .combine = bind_rows) %:%
  ...
  foreach(i = 1:100, .combine = bind_rows, .errorhandling = "remove") %do% {
    jack <- readRDS(sprintf("Output/pass_true_r_null/2016-11-30/out/jack.%s.%s.%s.rds", wt, r, i))
    jack %>% 
      as.data.frame() %>% 
      tibble::rownames_to_column(var = "group") %>% 
      tidyr::gather("var", "value", -group) %>% 
      mutate(index = i, wt = (wt == "jack_wt"), r = r)
  }
.tmp %>% 
  tidyr::spread(var, value) %>% 
  mutate(cover_zero = (estim - 2*se < 0) & (estim + 2*se > 0),
...
.tmp <- foreach(r = c("emp", "true"), .combine = bind_rows) %:%
  foreach(wt = c("nowt", "wt"), .combine = bind_rows) %:%
  foreach(i = 1:100, .combine = bind_rows, .errorhandling = "remove") %do% {
    jack <- readRDS(sprintf("Output/pass_true_r_non_null/2016-11-30/out/jack.%s.%s.%s.rds", wt, r, i))
    jack %>% 
      as.data.frame() %>% 
      tibble::rownames_to_column(var = "group") %>% 
      tidyr::gather("var", "value", -group) %>% 
